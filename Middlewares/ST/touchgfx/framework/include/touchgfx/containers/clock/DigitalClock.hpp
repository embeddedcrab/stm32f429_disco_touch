...
public:
    ...
    virtual bool getDisplayLeadingZeroForHourIndicator( void ) const
	{
    	return useLeadingZeroForHourIndicator;
	}

    virtual Unicode::UnicodeChar* getBuffer( void )
    {
    	return buffer;
    }

    virtual TextAreaWithOneWildcard& getText( void )
    {
    	return text;
    }
    ...
    virtual void setCurrentHour( const uint8_t& val );
    virtual void setCurrentMinute( const uint8_t& val );
    virtual void setCurrentSecond( const uint8_t& val );
